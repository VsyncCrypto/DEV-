...
bool CVsxStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CVsxStake from zerocoin spend", __func__);
...
bool CVsxStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CVsxStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CVsxStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CVsxStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CVsxStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CVsxStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CVsxStake::GetUniqueness() const
{
    //The unique identifier for a VSX stake is the outpoint
...
CBlockIndex* CVsxStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CVsxStake::ContextCheck(int nHeight, uint32_t nTime)
{
