...
{
    CDataStream serializedCoinSpend = ZVSXModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZVSXSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
...
bool UpdateZVSXSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
