...
bool CLegacyZVsxStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZVsxStake from non zc-spend", __func__);
...
CLegacyZVsxStake::CLegacyZVsxStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZVsxStake::GetIndexFrom()
{
...
CAmount CLegacyZVsxStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZVsxStake::GetUniqueness() const
{
...
bool CLegacyZVsxStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
