...
bool CzVSXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVSXCount(nCountLastUsed);
    else if (!walletdb.ReadZVSXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVSXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVSX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVSXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVSX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzVSXWallet::SeedToZVSX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
void CzVSXWallet::GenerateDeterministicZVSX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
