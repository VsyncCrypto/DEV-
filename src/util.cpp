...
const char * const VSYNC_CONF_FILENAME = "vsync.conf";
const char * const VSYNC_PID_FILENAME = "vsync.pid";
const char * const VSYNC_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VSYNCParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VSYNCParams
    // Mac: ~/Library/Application Support/VSYNCParams
    // Unix: ~/.vsync-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VSYNCParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "VSYNCParams";
#else
    // Unix
    return pathRet / ".vsync-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", VSYNC_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", VSYNC_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", VSYNC_PID_FILENAME));
