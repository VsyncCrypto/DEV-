...
public:
    ...
    bool WriteZVSXSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZVSXSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZVSXSeed_deprecated(uint256& seed);
    bool EraseZVSXSeed();
    bool EraseZVSXSeed_deprecated();

    bool WriteZVSXCount(const uint32_t& nCount);
    bool ReadZVSXCount(uint32_t& nCount);
