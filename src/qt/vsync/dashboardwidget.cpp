    ...
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareVsx, "square-chart-vsx");
    setCssProperty(ui->labelSquarezVsx, "square-chart-zvsx");
    setCssProperty(ui->labelVsx, "text-chart-vsx");
    setCssProperty(ui->labelZvsx, "text-chart-zvsx");
    ...
    setCssProperty(ui->labelAmountVsx, "text-stake-vsx-disable");
    setCssProperty(ui->labelAmountZvsx, "text-stake-zvsx-disable");
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isVsx = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZVSX;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair VSX, zVSX
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            vsx = (pair.first != 0) ? pair.first / 100000000 : 0;
            zvsx = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalVsx += pair.first;
            chartData->totalZvsx += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesVsx.append(vsx);
        chartData->valueszVsx.append(zvsx);

        int max = std::max(vsx, zvsx);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesVsx);
    set1->append(chartData->valueszVsx);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalVsx > 0 || chartData->totalZvsx > 0) {
        setCssProperty(ui->labelAmountVsx, "text-stake-vsx");
        setCssProperty(ui->labelAmountZvsx, "text-stake-zvsx");
    } else {
        setCssProperty(ui->labelAmountVsx, "text-stake-vsx-disable");
        setCssProperty(ui->labelAmountZvsx, "text-stake-zvsx-disable");
    }
    forceUpdateStyle({ui->labelAmountVsx, ui->labelAmountZvsx});
    ui->labelAmountVsx->setText(GUIUtil::formatBalance(chartData->totalVsx, nDisplayUnit));
    ui->labelAmountZvsx->setText(GUIUtil::formatBalance(chartData->totalZvsx, nDisplayUnit, true));
