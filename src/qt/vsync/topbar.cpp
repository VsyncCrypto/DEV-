    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopVsx}, "amount-small-topbar");
    setCssProperty({ui->labelAmountVsx}, "amount-topbar");
    setCssProperty({ui->labelPendingVsx, ui->labelImmatureVsx}, "amount-small-topbar");
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // VSX Total
    QString totalVsx = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // VSX
    // Top
    ui->labelAmountTopVsx->setText(totalVsx);
    // Expanded
    ui->labelAmountVsx->setText(totalVsx);
    ui->labelPendingVsx->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureVsx->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
