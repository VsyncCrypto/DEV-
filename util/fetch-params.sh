...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/VSYNCParams"
    else
        PARAMS_DIR="$HOME/.vsync-params"
    fi
fi
...
Failed to fetch the VSYNC zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
VSYNC - fetch-params.sh

This script will fetch the VSYNC zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common VSYNC zkSNARK parameters. Note that it is
