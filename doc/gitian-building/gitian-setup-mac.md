...
The build script we'll be using is contained in the VSYNC github repository ([contrib/gitian-build.py](https://github.com/vsync-project/vsync/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the VSYNC repository and will need to fetch this script with the following commands:
