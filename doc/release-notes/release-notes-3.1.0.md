VSYNC Core version *3.1.0* is now available from:  <https://github.com/vsync-project/vsync/releases>
...
VSX/zVSX Staking and Masternode Rewards
--------------

### VSX, zVSX and Masternode Payment Schedule
...
- 3 zVSX (3 x 1 denominations) rewarded to the staker, 2 VSX rewarded to the masternode owner and 1 VSX available for the budget. This is a total block reward of 6 VSX, up from 5.

If a user staking VSX wins the reward, the following amounts will be rewarded: 
- 2 VSX to the VSX staker, 3 VSX to the Masternode owner and 1 VSX available for the budget. This is a total block reward of 6 VSX, up from 5.


### Return change to sender when minting zVSX

Previously, zVSX minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most VSX and return the change from a zVSX mint to that address.
...
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular VSX.
...
As voted on by the VSYNC masternodes, the monthly budget available to be utilised has been increased to 42,000 VSX / month. This VSX only has the opportunity to be raised once per month (paid to winning proposals) with any unused VSX not created by the blockchain.

### Budget Finalization Fee

The VSYNC finalization fee for successful proposals has now been reduced, this fee is now 5 VSX down from 50 VSX. The total fee outlay for a successful proposal is now a total of 55 VSX.
...
Zerocoins, or zVSX, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zVSX that can be recalculated at any time using the seed. Deterministic zVSX allows for users to backup all of their future zVSX by simply recording their seed and keeping it in a safe place (similar to backing up a private key for VSX). The zVSX seed needs to remain in the wallet in order to spend the zVSX after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zVSX data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.
